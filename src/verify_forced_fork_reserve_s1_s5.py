#!/usr/bin/env python3
"""Verify a parent-intrinsic forced-fork reserve on S1 through S5.
...
Grouping the forced centers by step gives a schedule-independent lower bound
on total middle-fiber occurrence mass. The actual minimum center for a step
may come from an additional selected progression, so the verifier minimizes
the forced-center contribution over every initially possible center that could
be the minimum.
"""
...
def verify() -> str:
    lines = [
        "verified: root-forced coordinated actions on S1 through S5",
    ...
    lines.extend(
        [
            (
                ...
                "recorded state S1 through S5 has total middle-fiber "
