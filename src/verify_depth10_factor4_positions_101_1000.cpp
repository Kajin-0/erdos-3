...
constexpr int PRIOR_COUNT=100,EXPECTED_COUNT=900,EXPECTED_TOTAL=1000;
constexpr int EXPECTED_FIRST=76587092,EXPECTED_LAST=76603903;
constexpr long long TRIAL_CAP=3000000;
constexpr uint64_t EXPECTED_WITNESS_FNV=0xdc92ceac26789b47ULL;
...
int main(int argc,char**argv){try{
 ...
 for(int i=0;i<(int)a8.size();i++)for(int j=i+1;j<(int)a8.size();j++)setb(d8,a8[j]-a8[i]);
 auto s9=tr(raw(s8,R8),L9);int maxa9=s9.back();vector<uint64_t>d9((maxa9+64LL)/64);for(int x:s9)setb(d9,x);setb(d9,R8);setb(d9,2LL*R8);for(int d=1;d<=maxa8;d++)if(bit(d8,d)){setb(d9,d);setb(d9,1LL*R8+d);setb(d9,llabs(1LL*R8-d));setb(d9,2LL*R8+d);setb(d9,llabs(2LL*R8-d));}
 ...
 vector<uint64_t>d10((2LL*EXPECTED_LAST+64)/64);for(int x:s10)setb(d10,x);setb(d10,R9);setb(d10,2LL*R9);for(int d=1;d<=maxa9;d++)if(bit(d9,d)){setb(d10,d);setb(d10,1LL*R9+d);setb(d10,llabs(1LL*R9-d));setb(d10,2LL*R9+d);setb(d10,llabs(2LL*R9-d));}
 vector<int>domain;for(int R=INHERITED_MAX+1;R<=EXPECTED_LAST;R++){if(v2(R)%2)continue;if(bit(d10,R)||bit(d10,2LL*R))continue;domain.push_back(R);}if(domain.size()!=EXPECTED_TOTAL||domain[PRIOR_COUNT]!=EXPECTED_FIRST||domain.back()!=EXPECTED_LAST)throw runtime_error("domain prefix mismatch");
 ...
 string certificate;certificate.reserve(45000);long long total_trials=0,max_trials=0;int max_trial_R=0;
 for(int index=PRIOR_COUNT;index<EXPECTED_TOTAL;index++){
  int R=domain[index];RNG rng;rng.x^=(uint64_t)R*0x9e3779b97f4a7c15ULL;bool found=false;long long a=0,q=0,used=0;
  for(long long trial=0;trial<TRIAL_CAP;trial++){used=trial+1;long long x=B[rng.next()%B.size()]+(rng.next()%3)*1LL*R;long long y=B[rng.next()%B.size()]+(rng.next()%3)*1LL*R;if(x==y)continue;if(x>y)swap(x,y);long long step=y-x;if(in_candidate(x+2*step,R)&&in_candidate(x+3*step,R)){a=x;q=step;found=true;break;}}
  if(!found)throw runtime_error("deterministic witness search exhausted trial cap at R="+to_string(R));
  if(!in_candidate(a,R)||!in_candidate(a+q,R)||!in_candidate(a+2*q,R)||!in_candidate(a+3*q,R))throw runtime_error("generated point absent");
  certificate+=to_string(R)+" "+to_string(a)+" "+to_string(a+q)+" "+to_string(a+2*q)+" "+to_string(a+3*q)+"\n";
  total_trials+=used;if(used>max_trials){max_trials=used;max_trial_R=R;}
 }
 ...
 cout<<"verified: S10 factor-four domain positions 101-1000\n";
 cout<<"first_R="<<EXPECTED_FIRST<<"\nlast_R="<<EXPECTED_LAST<<"\nexplicit_witnesses="<<EXPECTED_COUNT<<"\n";
 cout<<"witness_fnv64=dc92ceac26789b47\n";
